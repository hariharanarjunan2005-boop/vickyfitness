...
export default function Facilities() {
  return (
    <section id="facilities" className="py-14 sm:py-24 bg-brand-surface border-y border-white/10 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Editorial Facility Mobile Stack */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center mb-6 sm:mb-16">
          ...
          <div className="lg:col-span-7 relative group rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
            <img
              ...
              className="w-full h-[260px] xs:h-[320px] sm:h-[500px] object-cover group-hover:scale-105 transition-transform duration-700"
              loading="lazy"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-brand-dark via-brand-dark/30 to-transparent" />
            
            <div className="absolute bottom-4 left-4 right-4 sm:bottom-6 sm:left-6 sm:right-6 p-4 sm:p-6 rounded-xl glass-panel border border-white/15">
              <span className="text-[10px] sm:text-xs font-bold text-brand-gold uppercase tracking-widest block mb-1">
                HEAVY TRAINING ZONE
              </span>
          ...
          <div className="lg:col-span-5 flex flex-col gap-4 sm:gap-6">
            {gymData.facilities.map((fac, idx) => (
              <div
                key={idx}
                className="p-4 sm:p-6 rounded-2xl bg-brand-dark border border-white/10 hover:border-brand-red/40 transition-all duration-300 hover:translate-x-1"
              >
                <div className="flex items-start gap-3.5 sm:gap-4">
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-brand-red/10 border border-brand-red/30 flex items-center justify-center text-brand-red font-bold text-xs sm:text-sm shrink-0">
                    0{idx + 1}
                  </div>
                  <div>
